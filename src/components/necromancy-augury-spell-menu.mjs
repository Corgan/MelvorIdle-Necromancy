...
export class NecromancyAugurySpellMenuElement extends HTMLElement {
    constructor() {
        super();
        this.requirements = [];
        this.spells = [];
        ...
        this._content.append(getTemplateNode('necromancy-augury-spell-menu-template'));
    ...
    init() {
        this.setSpells(game.necromancy.auguries.allObjects.sort((a,b)=>a.level - b.level));
    }
    getToggleCallback(spell) {
        return ()=>game.necromancy.selectAugurySpell(spell);
    }
    setSpellButton(spell, spellElem) {
        spellElem.setAugurySpell(spell);
    }
...
window.customElements.define('necromancy-augury-spell-menu', NecromancyAugurySpellMenuElement);