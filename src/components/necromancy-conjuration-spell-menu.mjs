...
export class NecromancyConjurationSpellMenuElement extends HTMLElement {
    constructor() {
        super();
        this.requirements = [];
        this.spells = [];
        ...
        this._content.append(getTemplateNode('necromancy-conjuration-spell-menu-template'));
    ...
    init() {
        this.setSpells(game.necromancy.conjurations.allObjects.sort((a,b)=>a.level - b.level));
    }
    getToggleCallback(spell) {
        return ()=>game.necromancy.selectConjurationSpell(spell);
    }
    setSpellButton(spell, spellElem) {
        spellElem.setConjurationSpell(spell);
    }
...
window.customElements.define('necromancy-conjuration-spell-menu', NecromancyConjurationSpellMenuElement);